<template>
    <b-card header="List of branches" header-tag="h4" class="bg-success-card">
        <v-client-table :data="tableData" :columns="columns" :options="options">
            <div slot="actions" slot-scope="props">

                <b-button-group size="sm">
                    <b-button variant="success" @click="edit(props.row.id)">Edit</b-button>
                    <b-button variant="danger" @click="deleteBranch(props.row.id)">Delete</b-button>
                </b-button-group>
            </div>
        </v-client-table>
    </b-card>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                tableData: [],
                columns: ['id', 'date', 'customer', 'status','actions'],
                options: {
                    sortIcon: {
                        base: 'fa',
                        up: 'fa fa-angle-up',
...
//                this.$router.push(`/admin/branches/${id}/edit`);
            ...
            deleteBranch: function (id) {
                console.log(id)
                axios.delete(`/api/manager/appointments/${id}/destroy?token=${this.token}`)
                        .then(res=> {
